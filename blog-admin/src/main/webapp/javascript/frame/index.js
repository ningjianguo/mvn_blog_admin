(function($) {
		...
		loadMenu: function(data) {
			...
			$.each(data, function(i) {
				...
				if(row.categoryParentid == "0") {
					...
					_html += '<i class="' + row.categoryIcon + '"></i><span>' + row.categoryName + '</span><i class="fa fa-angle-left pull-right"></i>'
					_html += '</a>'
					var childNodes = $.learunindex.jsonWhere(data, function(v) {
						return v.categoryParentid == row.categoryId
					});
					if(childNodes.length > 0) {
						...
						$.each(childNodes, function(i) {
							...
							var subchildNodes = $.learunindex.jsonWhere(data, function(v) {
								return v.categoryParentid == subrow.categoryId
							});
							_html += '<li>';
							if(subchildNodes.length > 0) {
								_html += '<a href="#"><i class="' + subrow.categoryIcon + '"></i>' + subrow.categoryName + '';
								_html += '<i class="fa fa-angle-left pull-right"></i></a>';
								_html += '<ul class="treeview-menu">';
								$.each(subchildNodes, function(i) {
									var subchildNodesrow = subchildNodes[i];
									_html += '<li><a class="menuItem" data-id="' + subrow.categoryId + '" href="' + subrow.categoryUrl + '"><i class="' + subchildNodesrow.categoryIcon + '"></i>' + subchildNodesrow.categoryName + '</a></li>';
								});
								_html += '</ul>';

							} else {
								_html += '<a class="menuItem" data-id="' + subrow.categoryId + '" href="' + subrow.categoryUrl + '"><i class="' + subrow.categoryIcon + '"></i>' + subrow.categoryName + '</a>';
							}
							_html += '</li>';
						});
						_html += '</ul>';
	...
	$(function() {
		$.learunindex.load();
		//$.learunindex.loadMenu(getCategories());
		getCategories();
		$.learuntab.init();
	});
})(jQuery);
...
function getCategories(){
	...
	$.ajax({
		...
		data:null,
		dataType:"json",
		async:false,//设置为同步请求,否则目录请求无法正常显示
		error:function(info){
			alert("获取目录失败!");
		},
		success:function(data){
			$.learunindex.loadMenu(eval(data.categories));
		}
	});
}