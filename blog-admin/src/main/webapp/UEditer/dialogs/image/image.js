...
//                }
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="upfile" id="upfile" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            '</form>' +
        ...
        uploadComplete: function(imgName){
            var me = this;
//            var json = eval(r);
           // alert(r);
           //alert(json.imgName);
//            alert(json);
            /*
             * {"state":"success","msg":"评论成功！"}
             */
            //var img = JSON.parse(imgName);
            //alert(img);
            //截取指定字符串的名字
            var img = imgName.substr(0, imgName.lastIndexOf('<'));
            img = img.substr(59, img.lastIndexOf('g'));
            var path1 = "../file/jjblog/editerimgs/"+img;
            var path = "<img alt='"+img+"' src='"+path1+"'>";
            UM.getEditor('myEditor').setContent(path, true);//图片显示在编辑框中
           /* try{
            	//var json = eval('{'+r+'}'); 
            	var json = eval("("+r+")");
